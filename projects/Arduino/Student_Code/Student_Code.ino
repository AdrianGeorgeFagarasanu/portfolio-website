...
void setup(){
   ...
   * Setup a pin mode like this: pinMode(macro name, INPUT/OUTPUT). 
...
void loop(){
     ...
     if(digitalRead(pirPin) == HIGH && !motion && !taskAccomplished){
          ...
          * Make the buzzer 1000Hz frequency, and the duration 1000 milliseconds.
          * __________________________________________________________________________________________________________
          */

          
         if (counter == 0){
                                    //Place buzzer code right before this comment.
           taskAccomplished = true;
         }
        }
 
     if(digitalRead(pirPin) == LOW){      
       digitalWrite(11, LOW);  //the led visualizes the sensors output pin state
       digitalWrite(12, LOW);
       digitalWrite(13, LOW);
